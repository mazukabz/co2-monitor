...
### v2.3.0 (aea319e)
- **Smart display caching**: кэширование данных для дисплея на 15 секунд
  - Устраняет мерцание когда датчик временно не готов (data_ready=false)
  - Показывает "SENSOR ERROR" только если данных нет более 15 секунд
  - MQTT отправляет только свежие данные (никогда кэш)

### v2.2.0 (6be36e2)
- Ручная калибровка CO2 через MQTT команду `calibrate`
- Калибровка на 420 ppm (текущий атмосферный уровень)

### v2.1.1 (1b35b6f)
- Убрана валидация данных — реальные значения всегда
- SCD41 имеет встроенную фильтрацию, дополнительная не нужна
